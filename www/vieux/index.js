...
function initSMSReceiver() {
    ...
    // === MÉTHODE 3: SMS.enableIntercept + TOUS LES EVENTS ===
...
function demarrerCoureur() {
    ...
    if (modeApp === 'depart') {
        ...
        if (typeof sms !== 'undefined' && sms.send) {
            ...
            console.log('Envoi SMS a:', telephoneArrivee);
            console.log('Message:', message);
            ...
            sms.send(telephoneArrivee, message, options, function() {
                ...
                showToast('Depart envoye par SMS!');
                updateDepartsListe();
            }, function(error) {
                console.error('Erreur SMS:', error);
                showToast('Erreur SMS: ' + error);
                updateDepartsListe();
            });
            ...
            showToast('Plugin SMS non disponible');
            console.log('SMS mode test:', message);
            updateDepartsListe();
        }
...
function updateCoureursActifs() {
    // En mode DÉPART, cacher la section
...
function updateListe() {
 ...
 var html = '';
    sorted.forEach(function(arrivant, index) {
        ...
        html += '<div class="arrivant-nom">#' + arrivant.numero + ' - ' + arrivant.nom + '</div>';
